"""Utilitaires métier VTC : export calendrier, notifications, nom d'affichage."""
...
def label_compte(c) -> str:
    ...
    La base ne stocke que l'e-mail ; le nom d'affichage vit dans la table
    métier `vtc_profils` (colonne jointe en alias `nom` par `conducteurs_actifs`,
    ou résolue ici via `nom_profil` quand on ne dispose que de l'id).
    """
    nom = (_course_get(c, "nom") or "").strip()
    if not nom:
        cid = _course_get(c, "id")
        if cid is not None:
            from .courses import nom_profil
            nom = (nom_profil(cid) or "").strip()
    if nom:
        return nom
